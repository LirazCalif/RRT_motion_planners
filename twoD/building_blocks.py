...
class BuildingBlocks2D(object):
    ...
    def compute_distance(self, prev_config, next_config):
        ...
        '''
        # TODO: HW2 4.2.1
        #compute norm between two configurations
    ...
    def sample_random_config(self, goal_prob, goal):
        ...
        sample=np.random.uniform(low=-np.pi, high=np.pi,size=self.dim)
        return sample
    ...
    def compute_forward_kinematics(self, given_config):
        ...
        for i in range(self.dim):
            ...
            x = self.links[i] * np.cos(ee_angle) + prev_xy[0]
            y = self.links[i] * np.sin(ee_angle) + prev_xy[1]
    ...
    def validate_robot(self, robot_positions):
        ...
        # TODO: HW2 4.2.3
        links=[]
        #if the robot positions wasn't initialize with (0,0) add (0,0)
        if robot_positions[0][0]!=0 and robot_positions[0][1]!=0:
            robot_positions = np.concatenate([np.zeros((1, 2)), robot_positions])
        #for each link, check intersections with all other non-adjacent links
        for i in range(self.dim):
            links.append(LineString([tuple(robot_positions[i]), tuple(robot_positions[i+1])]))
        for i in range(self.dim):
            for j in range(i+2,self.dim):
    ...
    def get_inspected_points(self, config):
        ...
        # print("the forward position of the robot are:", ee_pos)
    ...
    def compute_union_of_points(self, points1, points2):
        ...
        '''
        # TODO: HW3 2.3.2
        set1 = set(map(tuple, points1)) if points1 is not None else set()

        set2 = set(map(tuple, points2)) if points2 is not None else set()
    ...
    def inverse_kinematics(self, target, max_iters=50, tol=1e-2, alpha=0.5, initial_guess=None):
        """
        Numerical IK using Jacobian pseudo-inverse for planar n-link arm.
        Returns a configuration reaching target if found, else None.

        @param target: np.array([x, y]) end-effector target
        """
        if initial_guess is None:
            config = np.random.uniform(-np.pi, np.pi, size=self.dim)
        else:
            config = initial_guess.copy()

        for _ in range(max_iters):
            # Forward kinematics
            joints = self.compute_forward_kinematics(config)
            ee_pos = joints[-1]
            ...
            # Compute planar Jacobian (2 x n)
            J = np.zeros((2, self.dim))
            for i in range(self.dim):
                joint_pos = np.zeros(2) if i == 0 else joints[i - 1]
                r = ee_pos - joint_pos
                # 2D Jacobian column: [-y, x]^T for rotation about joint
                J[:, i] = np.array([-r[1], r[0]])

            # Pseudo-inverse of Jacobian
            J_pinv = np.linalg.pinv(J)
            d_theta = alpha * J_pinv @ delta

            # Update configuration
            config += d_theta
            config = (config + np.pi) % (2 * np.pi) - np.pi
