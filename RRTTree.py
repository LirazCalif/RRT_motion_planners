...
class RRTTree(object):
    ...
    def add_vertex(self, config, inspected_points=None):
        ...
        if self.task == "ip":
            ee = self.bb.compute_forward_kinematics(config)[-1]
            self.vertices[vid] = RRTVertex(config=config, inspected_points=inspected_points,ee_pos=ee)
        else:
...
class RRTVertex(object):
    ...
    def __init__(self, config, cost=0, inspected_points=None,ee_pos=None):
        self.config = config
        self.cost = cost
